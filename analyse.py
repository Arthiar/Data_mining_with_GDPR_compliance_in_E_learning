# analyse.py — Organise outputs per activity type, generate insightful charts, write summaries.
# Run AFTER export.py (or after generate_demo_data.py for demonstration purposes). It will:
#   - Create folders: output/quizzes, /assignments, /h5p, /forums, /databases, /summary
#   - MOVE CSVs from output/exports/ to the right folder
#   - Create charts beside each CSV + cross-activity comparison charts in /summary
#   - Write per-type summaries + a global overview
...
PASS_THRESHOLD = 50.0   # score_pct >= this is considered "passing"


# ── Utility helpers ────────────────────────────────────────────────────────────

def _move_from_exports(src: Path, dst_dir: Path) -> Path:
...
def _caption(fig, text: str):
    """Add a small italic explanation beneath the chart area."""
    fig.text(
        0.5, 0.005,
...
def _save_hist(series, title, xlabel, outpath, pass_line: bool = False, note: str = ""):
    """Histogram with mean/median lines and optional pass-rate annotation."""
    s = pd.to_numeric(series, errors="coerce").dropna()
    if s.empty:
        return False

    fig, ax = plt.subplots(figsize=(9, 5.4))
    ax.hist(s, bins=10, color=_BLUE, alpha=0.85, edgecolor="white", linewidth=0.8)

    mean_val   = s.mean()
    median_val = s.median()
    ax.axvline(mean_val,   color=_RED,    linewidth=2, linestyle="--",
               label=f"Mean {mean_val:.1f}")
    ax.axvline(median_val, color=_ORANGE, linewidth=2, linestyle=":",
               label=f"Median {median_val:.1f}")

    if pass_line and s.max() <= 101:
        ax.axvline(PASS_THRESHOLD, color=_GREEN, linewidth=2, linestyle="-.",
        ...
        ax.text(PASS_THRESHOLD + (s.max() - s.min()) * 0.03,
                y_top * 0.88,
                f"Pass rate\n{pct:.1f}%  ({n_pass}/{len(s)})",
                fontsize=9.5, color=_GREEN, fontweight="bold")
...
def _save_scatter(x, y, title, xlabel, ylabel, outpath, note: str = ""):
    """Scatter with trend line and correlation label."""
    ...
    ax.scatter(df["x"], df["y"], color=_BLUE, alpha=0.75,
    ...
    if len(df) >= 3:
        ...
        ax.plot(x_ln, poly(x_ln), color=_GREEN, linewidth=2, linestyle="--",
                label=f"Trend  r = {corr_r:+.2f}")
        ax.legend()

        strength  = ("strong" if abs(corr_r) >= 0.6
                     else "moderate" if abs(corr_r) >= 0.3 else "weak")
...
def _save_bar_from_counts(counts: pd.Series, title, xlabel, ylabel, outpath, note: str = ""):
    """Bar chart (auto-horizontal for >8 categories) with value labels."""
    ...
    if horizontal:
        bars = ax.barh(labels, values, color=_BLUE, alpha=0.85,
        ...
        bars = ax.bar(labels, values, color=_BLUE, alpha=0.85,
                      edgecolor="white", linewidth=0.8)
...
def _save_pass_fail_bar(n_pass: int, n_fail: int, title: str, outpath: Path,
                        note: str = "") -> bool:
    """Stacked horizontal bar showing pass/fail split with percentages."""
    total = n_pass + n_fail
    if total == 0:
        return False
    ...
    ax.barh([""], [pct_pass], color=_GREEN, edgecolor="white",
            label=f"Pass  {pct_pass:.1f}%  (n={n_pass})")
    ax.barh([""], [pct_fail], left=[pct_pass], color=_RED, edgecolor="white",
...
def _save_grouped_comparison(names, values, title, ylabel, outpath,
                              color=_BLUE, ref_line=None, note: str = ""):
    """Bar chart comparing a single metric across multiple activities."""
    ...
    labels = [_wrap(str(n), 20) for n in names_f]
    ...
    if ref_line is not None:
        ax.axhline(ref_line, color=_ORANGE, linewidth=1.8, linestyle="--",
...
def analyze_quizzes():
    ...
    for csv in sorted(EXPORTS.glob("quiz_*.csv")):
        local_csv = _move_from_exports(csv, QUIZ_DIR)
        df = pd.read_csv(local_csv)
        _analyze_single_quiz(df, local_csv, summaries)

    # Also process CSVs already in QUIZ_DIR (from a previous run or demo data)
    already_processed = {local_csv.name for local_csv in QUIZ_DIR.glob("quiz_*.csv")}
    for csv in sorted(QUIZ_DIR.glob("quiz_*.csv")):
        df = pd.read_csv(csv)
        if not summaries or not any(s["cmid"] == str(csv.stem.split("_", 1)[1] if "_" in csv.stem else "unknown") for s in summaries):
            _analyze_single_quiz(df, csv, summaries)

    # Deduplicate summaries by cmid
    seen, deduped = set(), []
    for s in summaries:
        if s["cmid"] not in seen:
            seen.add(s["cmid"])
            deduped.append(s)
    summaries = deduped
    ...
    if summaries:
        ...
        _save_grouped_comparison(
            ...
            color=_BLUE, ref_line=PASS_THRESHOLD,
        ...
        if df_sum["mean_duration_min"].notna().any():
            _save_grouped_comparison(
                ...
                SUMMARY_DIR / "quizzes_duration_comparison.png", color=_ORANGE,
...
def _analyze_single_quiz(df: pd.DataFrame, local_csv: Path, summaries: list):
    ...
    score_s = _numeric(df, "score_pct")
    dur_s   = _numeric(df, "duration_minutes")

    _save_hist(
        score_s, f"{quiz_name}\nScore Distribution", "Score (%)",
        QUIZ_DIR / f"{local_csv.stem}_score_hist.png",
        pass_line=True,
    ...
    if not score_s.empty:
        ...
        _save_pass_fail_bar(
            n_pass, n_fail,
            f"{quiz_name}\nPass / Fail Split",
            QUIZ_DIR / f"{local_csv.stem}_pass_fail.png",
    ...
    _save_hist(
        ...
        QUIZ_DIR / f"{local_csv.stem}_duration_hist.png",
    ...
    _save_scatter(
        ...
        "Duration (min)", "Score (%)",
        QUIZ_DIR / f"{local_csv.stem}_time_vs_score.png",
    ...
    pass_rate = _percent(n_pass_r, len(score_s)) if n_pass_r is not None else None
...
def analyze_assignments():
    ...
    for csv in sorted(EXPORTS.glob("assign_*.csv")):
        local_csv = _move_from_exports(csv, ASSIGN_DIR)
        df = pd.read_csv(local_csv)
        _analyze_single_assignment(df, local_csv, summaries)

    for csv in sorted(ASSIGN_DIR.glob("assign_*.csv")):
        df = pd.read_csv(csv)
        cmid = csv.stem.split("_", 1)[1] if "_" in csv.stem else "unknown"
        if not any(s["cmid"] == cmid for s in summaries):
            _analyze_single_assignment(df, csv, summaries)
    ...
    if summaries:
        ...
        if df_sum["mean_grade"].notna().any():
            _save_grouped_comparison(
                ...
                SUMMARY_DIR / "assignments_mean_grade_comparison.png", color=_BLUE,
...
def _analyze_single_assignment(df: pd.DataFrame, local_csv: Path, summaries: list):
    cmid  = local_csv.stem.split("_", 1)[1] if "_" in local_csv.stem else "unknown"
    aname = (df["assign_name"].dropna().iloc[0]
             if "assign_name" in df.columns and not df["assign_name"].dropna().empty
             else f"assign_{cmid}")

    grade_s = _numeric(df, "grade")
    # Filter out -1 (Moodle sentinel for "not graded")
    grade_s = grade_s[grade_s >= 0]
    time_s  = _numeric(df, "time_to_submit_hours")
    n_total = len(df)

    if not grade_s.empty:
        _save_hist(
            grade_s, f"{aname}\nGrade Distribution", "Grade (0–100)",
            ASSIGN_DIR / f"{local_csv.stem}_grade_hist.png",
    ...
    if not time_s.empty:
        _save_hist(
            ...
            ASSIGN_DIR / f"{local_csv.stem}_submit_timing_hist.png",
    ...
    if "duedate" in df.columns and "submitted_unix" in df.columns:
        ...
        if mask.any():
            ...
            ontime_rate = _percent(n_ontime, int(mask.sum()))
...
def analyze_h5p():
    ...
    for csv in sorted(EXPORTS.glob("h5p_*.csv")):
        local_csv = _move_from_exports(csv, H5P_DIR)
        df = pd.read_csv(local_csv)
...
def analyze_forums():
    ...
    for csv in sorted(EXPORTS.glob("forum_*_posts.csv")):
        local_posts = _move_from_exports(csv, FORUM_DIR)
    ...
    for csv in sorted(EXPORTS.glob("forum_*_summary.csv")):
        _move_from_exports(csv, FORUM_DIR)
...
def analyze_databases():
    ...
    for csv in sorted(EXPORTS.glob("data_*.csv")):
        local_csv = _move_from_exports(csv, DATA_DIR)
        df = pd.read_csv(local_csv)
...
# ── Global overview + cleanup ─────────────────────────────────────────────────

def summarize_overview_and_cleanup():
    categories = [
...
if __name__ == "__main__":
    ...
    analyze_forums()
    analyze_databases()
    summarize_overview_and_cleanup()
    print("Done. CSVs in type folders. Charts with captions beside each CSV. Summaries in 'output/summary/'.")
